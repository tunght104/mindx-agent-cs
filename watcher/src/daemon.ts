...
let config: any = {};
try {
  config = JSON.parse(fs.readFileSync(path.join(__dirname, "../.watcher.config.json"), "utf-8"));
} catch (err) {
  logger.warn("Could not read watcher.config.json. Using defaults.");
}
const ALLOWED_SENDERS = config.ALLOWED_SENDERS || [];
const MANUAL_TYPES = config.MANUAL_TYPES || [];
...
async function processEmail(messageId: string, subject: string, agentCliName: string): Promise<void> {
  ...
  if (ticketType === "unknown") {
    ...
    ticketType = await classifyWithAI(messageId, subject, odooContent, agentCliName);
...
async function pollOnce(agentCliName: string): Promise<void> {
  logger.info(`--- Poll cycle started (pageSize=${POLL_PAGE_SIZE}, dryRun=${DRY_RUN}, agentCli=${agentCliName}) ---`);
  ...
  for (const email of newEmails) {
    try {
      ...
      await processEmail(email.id, email.subject ?? "", agentCliName);
    } catch (err) {
...
async function main(agentCliName: string) {
  logger.info(`=== MindX Watcher Daemon starting (Agent CLI: ${agentCliName}) ===`);
  logger.info(`Poll interval: ${POLL_INTERVAL_MS / 1000}s | Page size: ${POLL_PAGE_SIZE} | Dry-run: ${DRY_RUN}`);

  // Run immediately on startup, then on interval
  await pollOnce(agentCliName);
...
export async function startDaemon(agentCliName: string = "agent") {
  return main(agentCliName);
}
